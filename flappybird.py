...
class Bird:
    ...
    def move(self):
        ...
        self.y = self.y + d
        #tilt mynd up eða niður eftir punkt
    ...
    def draw(self, win):
        self.img_count += 1
    #tekka hvaða mynd á að birta eftir img_count
        #ef img_count er minna en animationtime(5) þá birta mynd 0 o.s.frv
        if self.img_count < self.ANIMATION_TIME:
            ...
            self.img = self.IMGS[0]
            self.img_count = 0
        #þegar flappybird er að fara niður
        if self.tilt <= -80:
            self.img = self.IMGS[1]
...
class Pipe:
    ...
    def __init__(self, x):
        ...
        self.set_height()
    def set_height(self):
        #teikna pip a negative location
    ...
    #nota mask til að tekka a collision á ekki transparent pixels og ekki bara box
    def collide(self, bird):
        #gera mask fyrir bird og pipes
        bird_mask = bird.get_mask()
        top_mask = pygame.mask.from_surface(self.PIPE_TOP)
        bottom_mask = pygame.mask.from_surface(self.PIPE_BOTTOM)
        #reikna offset (hvað eru hlutirnir langt fra hvor öðrum?)
        top_offset = (self.x - bird.x, self.top - round(bird.y))
        bottom_offset = (self.x - bird.x, self.bottom - round(bird.y))
        #er collision?
        b_point = bird_mask.overlap(bottom_mask, bottom_offset)
        t_point = bird_mask.overlap(top_mask, top_offset)
        #tekka ef þessi punktar seu til?
